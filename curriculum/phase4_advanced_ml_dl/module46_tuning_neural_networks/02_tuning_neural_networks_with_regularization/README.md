...
![title](https://raw.githubusercontent.com/learn-co-curriculum/dsc-tuning-neural-networks-with-regularization/master/images/example.png)
...
![title](https://raw.githubusercontent.com/learn-co-curriculum/dsc-tuning-neural-networks-with-regularization/master/images/underfitting.png)
...
![title](https://raw.githubusercontent.com/learn-co-curriculum/dsc-tuning-neural-networks-with-regularization/master/images/good.png)
...
![title](https://raw.githubusercontent.com/learn-co-curriculum/dsc-tuning-neural-networks-with-regularization/master/images/overfitting.png)

## The Santa Example

![Drawing](https://raw.githubusercontent.com/learn-co-curriculum/dsc-tuning-neural-networks-with-regularization/master/images/S_4.jpg)

![Drawing](https://raw.githubusercontent.com/learn-co-curriculum/dsc-tuning-neural-networks-with-regularization/master/images/NS_1.jpg)
     High variance High bias High variance & bias Low variance and bias    train set error 12% 26% 26% 12%   validation set error 25% 28% 40% 13%
...
## Rules of Thumb Regarding Bias / Variance   High Bias? (training performance) High variance? (validation performance)    Use a bigger network More data   Train longer Regularization   Look for other existing NN architextures Look for other existing NN architextures
...
$$ J (w,b) = \dfrac{1}{m} \sum^m*{i=1}\mathcal{L}(\hat y^{(i)}, y^{(i)})+ \dfrac{\lambda}{2m}||w||*2^2$$

$$||w||*2^2 = \sum^{n*x}*{j=1}w*j^2= w^Tw$$
...
$$ J (w^{[1]},b^{[1]},...,w^{[L]},b^{[L]}) = \dfrac{1}{m} \sum^m*{i=1}\mathcal{L}(\hat y^{(i)}, y^{(i)})+ \dfrac{\lambda}{2m}\sum^L*{l=1}||w^{[l]}||^2$$
...
Whichever expression you have from the backpropagation, and add $\dfrac{\lambda}{m} w^{[l]}$. So,
...
Also: e.g., *tanh* function, if $w$ is small, the activation function will be mostly operating in the linear region and not "explode" as easily.
...
![title](https://raw.githubusercontent.com/learn-co-curriculum/dsc-tuning-neural-networks-with-regularization/master/images/dropout.png)
