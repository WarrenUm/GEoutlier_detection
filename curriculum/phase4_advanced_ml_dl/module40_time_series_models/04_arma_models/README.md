...
Or, mathematically: $$\large Y*t = \mu + \phi * Y*{t-1}+\epsilon_t$$

Some notes based on this formula: - If the slope is 0, the time series is a white noise model with mean $\mu$ - If the slope is not 0, the time series is autocorrelated - Bigger slope means bigger autocorrelation - When there is a negative slope, the time series follows an oscillatory process
...
Or, mathematically: $$\large Y*t = \mu +\epsilon*t + \theta * \epsilon_{t-1}$$

Some notes based on this formula: - If the slope is 0, the time series is a white noise model with mean $\mu$ - If the slope is not 0, the time series is autocorrelated and depends on the previous white noise process - Bigger slope means bigger autocorrelation - When there is a negative slope, the time series follow an oscillatory process
...
- AR: $Y*t = \mu + \phi * Y*{t-1}+\epsilon_t$
- MA: $Y*t = \mu +\epsilon*t + \theta * \epsilon_{t-1}$
...
- AR(2): $Y*t = \mu + \phi*1 * Y*{t-1}+\phi*2 * Y*{t-2}+\epsilon*t$
- MA(2): $Y*t = \mu +\epsilon*t + \theta*1 * \epsilon*{t-1}+ \theta*2 * \epsilon*{t-2}$
...
$$Y*t = \mu + \phi*1 Y*{t-1}+\phi*2 Y*{t-2}+ \theta \epsilon*{t-1}+\epsilon_t$$

A short table to summarize ACF and PACF for AR(p), MA(q), and ARMA(p, q):    AR(p) MA(q) ARMA(p, q)    ACF Tails off Cuts off after lag q Tails off   PACF Cuts off after lag p Tails off Tails off
...
- `winning_400m.csv` — supporting data/helper file for the exercise.
