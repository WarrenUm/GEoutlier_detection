...
$$cov(X,Y) = \frac{\sum*i^n(X*i -\mu*X)(Y*i - \mu_Y)}{n-1}$$
...
$$C^{n x n} = (c*{i,j}, c*{i,j} = cov(Dim*i, Dim*j))$$
...
![image](https://raw.githubusercontent.com/learn-co-curriculum/dsc-pca-covariance-matrix-eigendecomp/master/images/covmat.png)
...
The diagonal elements, $C*{ii}$ are the variances in the variables $x*i$ assuming $N−1$ degrees of freedom:
...
Here, **lambda** ($\lambda$) represents the **eigenvalue** scalar.
...
![image](https://raw.githubusercontent.com/learn-co-curriculum/dsc-pca-covariance-matrix-eigendecomp/master/images/new_eig1.png)
