...
Here are a few other types of data in Python that we will talk more about in later lessons: `python 100 # Integer 10.0 # Float True # Boolean `
...
In this lesson, we went through that cycle: * Guess: We just tried something and looked to the error message for clues as to what to do next. * help(str): We saw a nice way to learn about new methods, then we took a guess to test our understanding * Following a pattern: We started with a simple method like calling upper, took a moment to break this down into a pattern, and then tried this pattern again to call other methods
