...
> Note: More conventionally, these contrived examples would employ the `enumerate()` method, but that is beyond the scope of the current lesson. At some point in the future, examine how this code snippet works: ` for idx, item in enumerate(['A', 'B', 'C']): print(idx, item) `
...
- `cities.xlsx` — supporting data/helper file for the exercise.
