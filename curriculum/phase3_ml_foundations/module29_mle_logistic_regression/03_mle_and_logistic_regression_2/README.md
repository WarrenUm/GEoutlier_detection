...
$\pi*i = Pr(Y*i = 1|X*i = x*i)=\dfrac{\text{exp}(\beta*0 + \beta*1 x*i)}{1 + \text{exp}(\beta*0 + \beta*1 x*i)}$

This is the standard linear regression model ($\beta*0+\beta*1 x_i$) you have seen previously, modified to have a range of 0 to 1. The range is modified and constrained by applying the sigmoid function since you're predicting probabilities.
...
$ L(\beta*0,\beta*1)=\prod\limits*{i=1}^N \pi*i^{y*i}(1-\pi*i)^{n*i-y*i}=\prod\limits*{i=1}^N \dfrac{\text{exp}{y*i(\beta*0+\beta*1 x*i)}}{1+\text{exp}(\beta*0+\beta*1 x*i)}$
