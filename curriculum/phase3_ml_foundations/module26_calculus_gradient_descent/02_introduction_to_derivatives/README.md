...
![png](https://raw.githubusercontent.com/learn-co-curriculum/dsc-derivatives-intro/master/index_files/index_3_0.png)
...
![png](https://raw.githubusercontent.com/learn-co-curriculum/dsc-derivatives-intro/master/index_files/index_8_0.png)
...
![png](https://raw.githubusercontent.com/learn-co-curriculum/dsc-derivatives-intro/master/index_files/index_12_0.png)
...
- Another way of expressing **change in $y$** is:
  ...
  - $y*2 - y*1$ or $\Delta y$, read "delta y"

- Likewise, another way of expressing **change in $x$** is:

  - $x*2 - x*1$ or $\Delta x$, read "delta x"

Generally, we can say that the derivative of a straight line is equal to the "rise over the run":

- rate of change $= \dfrac{\text{rise}}{\text{run}} = \dfrac{\Delta y}{\Delta x} = \dfrac{y*2 - y*1}{x*2 - x*1}$

Just like in our example, we saw:

- miles per hour = $\dfrac{\text{distance}*2 - \text{distance}*1}{\text{time}*2 - \text{time}*1} = \dfrac{12 - 6}{2 - 1} = \dfrac{6}{1} = 6$ mph
...
- $ f'(x) = \dfrac{\text{rise}}{\text{run}} = \dfrac{\Delta y}{\Delta x} = \dfrac{y*2 - y*1}{x*2 - x*1} = \dfrac{f(x*2) - f(x*1)}{x*2 - x*1}$

Take a look to at the equation far to the right:

$$f'(x) = \frac{f(x*2) - f(x*1)}{x*2 - x*1} $$

You see that we replaced $y*2 - y*1$ with $f(x*2) - f(x*1)$. This makes sense, because really when we say $y*2$ and $y*1$, we mean the function's output at the first value of $x$ and the function's output at the second value of $x$.
...
So $f(x)$ equals the output at a given point. And $f'(x)$ is the rate of change at a given point. So then: * $f(1)$ means the output at $x = 1$, or in our example, *the distance* at hour one, and * $f'(1)$ means the rate of change at $x = 1$, or in our example, *the speed* at hour one

Because the jogger's speed never changes throughout and since the derivative is the rate of change at a given point, we can conclude that the derivative also never changes. That's right! Let's take $x*1=3$ and $x*2=4$:
...
![png](https://raw.githubusercontent.com/learn-co-curriculum/dsc-derivatives-intro/master/index_files/index_27_0.png)
...
![png](https://raw.githubusercontent.com/learn-co-curriculum/dsc-derivatives-intro/master/index_files/index_33_0.png)
...
![png](https://raw.githubusercontent.com/learn-co-curriculum/dsc-derivatives-intro/master/index_files/index_35_0.png)
...
$$ f'(x) = \frac{\Delta y}{\Delta x} = \frac{y*2 - y*1}{x*2 - x*1} $$

Then we saw that we can express the derivative in terms of $f(x)$ instead of $y*1$ and $y*2$ as in the output at second $x$ minus the output at the first x divided by the difference between the two $x$ values. Or, in an equation:
...
- `graph.py` — supporting data/helper file for the exercise.
