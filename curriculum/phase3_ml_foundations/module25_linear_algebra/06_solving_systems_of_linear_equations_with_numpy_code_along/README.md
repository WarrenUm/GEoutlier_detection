...
![image](https://raw.githubusercontent.com/learn-co-curriculum/dsc-lineq-numpy-codealong/master/images/diagonal.png)
...
The dot product for these two matrices can be calculated as: `python import numpy as np A = np.array([[2,1],[3,4]]) I = np.array([[1,0],[0,1]]) print(I.dot(A)) print('\n', A.dot(I)) `
...
NumPy comes with a built-in function `np.identity()` to create an identity matrix. Just pass in the dimension (number of rows or columns) as the argument. You can add an argument `dtype=int` to make sure the elements are integers (if not, your identity matrix will contain floats): `python print(np.identity(4, dtype=int)) print(np.identity(5, dtype=int)) `
...
![image](https://raw.githubusercontent.com/learn-co-curriculum/dsc-lineq-numpy-codealong/master/images/ab.png)

So first we'll need to calculate the inverse of the square matrix containing coefficient values: ```python

#  Define A and B

A = np.matrix([[2, 1], [3, 4]]) B = np.matrix([35, 65])

# Take the inverse of Matrix A

A*inv = np.linalg.inv(A) A*inv ```
...
The dot product of $A$ and $X$ should give matrix $B$. Let's try it: `python print(A.dot(X)) print(B) `
