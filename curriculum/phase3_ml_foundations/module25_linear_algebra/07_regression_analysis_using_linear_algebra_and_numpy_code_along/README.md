...
[You can find a deeper mathematical and geometric explanation of the topic here](http://math.mit.edu/%7Egs/linearalgebra/ila0403.pdf). In this lesson, we'll try to keep things more data-oriented.
...
![image](images/new_regr_line.png)
...
![image](https://raw.githubusercontent.com/learn-co-curriculum/dsc-linalg-regression-codealong/master/images/new_regression.png)
...
Let's use the above formula to calculate a solution for our toy problem: ```python

# Calculate the solution

X = np.array([[1, 1],[1, 2],[1, 3]]) y = np.array([1, 2, 2]) Xt = X.T XtX = Xt.dot(X) XtX*inv = np.linalg.inv(XtX) Xty = Xt.dot(y) x*hat = XtX*inv.dot(Xty) # the value for b shown above x*hat ```
...
![image](https://raw.githubusercontent.com/learn-co-curriculum/dsc-linalg-regression-codealong/master/./images/new_LinRegresChart.png)

When you have more than one input variable, each data point can be seen as a feature vector $x*i$, composed of $x*1, x*2, \ldots , x*m$ , where $m$ is the total number of features (columns). For multiple regression, each data point can contain two or more features of the input. To represent all of the input data along with the vector of output values we set up a input matrix *X* and an output vector *y*.
...
observed data $\rightarrow$ $y = b*0+b*1x*1+b*2x*2+ \ldots + b*px_p+ \epsilon $

predicted data $\rightarrow$ $\hat y = \hat b*0+\hat b*1x*1+\hat b*2x*2+ \ldots + \hat b*px_p $
...
[Simple Regression in Matrix format](https://www.stat.cmu.edu/%7Ecshalizi/mreg/15/lectures/13/lecture-13.pdf)
